...
def has_standalone_authority_record(comments: list[str]) -> bool:
    """Recognize only the durable, complete standalone approval record."""
...
def validate_implementation_authority_contract() -> None:
    ...
    if resolve_issue_authority(parent=None, comments=[approved_record]) != "standalone":
        fail("bounded approved standalone /to-tickets issue without a parent was not accepted")
    if resolve_issue_authority(parent=None, comments=[approved_record + "\n- extra: ignored"]) is not None:
    ...
    if resolve_issue_authority(parent="unapproved", comments=[approved_record]) is not None:
    ...
    if resolve_issue_authority(parent=None, comments=["\n".join((*STANDALONE_APPROVAL_FIELDS, STANDALONE_APPROVAL_PATHS[1]))]) is not None:
