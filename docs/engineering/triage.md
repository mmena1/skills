...
No. They are already agent-ready, because `to-tickets` applies the `ready-for-agent` label as it publishes, precisely so an AFK runner picks them up without another pass. Specs from `/to-spec` remain `needs-triage` until `/design-review` approves them. The spec flow meets `ready-for-agent` only when `/to-tickets` publishes executable slices.
