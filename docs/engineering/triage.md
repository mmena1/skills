...
No. They are already planned, and `to-tickets` applies the configured ready state only to the executable frontier. Blocked tickets remain non-ready until a successful `implement` closeout promotes them. Specs from `/to-spec` remain `needs-triage` until `/design-review` approves them.
...
Three shapes recur. A fully specified issue waiting on another issue ([#139](https://github.com/mattpocock/skills/issues/139)), trigger-gated future work that is not actionable yet ([#297](https://github.com/mattpocock/skills/issues/297)), and work awaiting verification. The planned-ticket flow now handles the first shape: `to-tickets` leaves blocked implementation tickets non-ready and `implement` promotes them only when their blockers close. Triage still has no general-purpose paused or awaiting-verification state for incoming issues, so repositories may need a local state for those other shapes.
...
Yes, the tracker is config, not a hard-coded assumption, and people run it against custom trackers such as Linear (via the `linear` CLI) and plain markdown files under `.scratch/`. A common split is a custom tracker for issues and planning, GitHub for code and PRs: skills that say "issue tracker" map to that configured system, skills that say "PR" map to GitHub. On the local-markdown tracker there is an open template bug where the generated file can carry the acceptance criteria twice, once at the top level and once inside the agent brief ([#200](https://github.com/mattpocock/skills/issues/200)).
