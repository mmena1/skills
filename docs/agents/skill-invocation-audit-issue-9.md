...
| Claude Code | `disable-model-invocation: true` | Model invocation enabled by omission; explicit use remains available | Claude documents `user-invocable: false` as hiding the skill from the `/` menu and not running it when the user types `/codebase-design`, while model invocation remains enabled |
...
The 2.1.283 results match the Claude Code controls for all three examples. The interactive `/` menu was not rechecked on 2.1.283, so the 2.1.282 autocomplete observation remains unexplained but is no longer reproduced. Claude automatic selection is demonstrated for `codebase-design` but not for `diagnosing-bugs`.
