...
- **Standalone authority record**: add a GitHub issue comment with exactly this structure after `/to-tickets` has published a parentless issue whose breakdown the user explicitly approved, or after `/triage` receives explicit maintainer approval both that no parent/spec is intended and that the issue itself is implementation authority:
  ...
  The recognized approval values are `/to-tickets` and `/triage`. Triage comments must retain their required AI disclaimer before the record. Verify the comment's author metadata and accept it only when `user.type` is `User` and `author_association` is `OWNER` or `COLLABORATOR`. `/implement` accepts this record only when the issue has no parent. A ready label or agent brief alone is insufficient.
