...
3. **Branch — is this a multi-session build?**
   ...
   Either way, **`/implement`** resolves the ticket and approved parent, passes the tracker and clean-worktree gates, then drives **`/tdd`** one red-green slice at a time. It commits before explicitly composing **`/code-review`** against the captured starting SHA, fixes and commits blocking findings, then closes the ticket and refreshes the frontier. Reach for **`/tdd`** on its own when you just want to build a concrete behaviour test-first without a full spec, and invoke **`/code-review`** yourself whenever you want to review a branch or PR against a fixed point; it is not an automatic reviewer.
