...
4. **Grill (if needed).** If the request needs fleshing out, apply `grilling`, then apply `domain-modeling`, and grill it into shape a round of questions at a time, sharpening domain terms and updating `CONTEXT.md`/ADRs inline as decisions land. Read either skill's `SKILL.md` from the active skill root first if it is not already loaded.
