...
- **PR/merge handoff and post-merge reconciliation**: the separately authorized PR/merge owner—the human or workflow that creates and merges the implementation PR—owns adding and verifying `Closes #<n>`. After merge, that owner confirms the issue auto-closed, then re-queries every open child of the parent and refreshes the frontier: add the implementation-ready label to every unblocked, unassigned child; remove it from blocked or assigned children; preserve parent order; and report the resulting frontier. If the issue did not auto-close, stop the reconciliation and report the missing or ineffective close reference rather than promoting dependent tickets. If the closed ticket belongs to a Wayfinder map, also perform its resolve operation below.
- **`/reconcile` contract**: `/reconcile <ticket-ref>` uses the resolved ticket as its trigger, follows its direct parent/spec, enumerates every implementation child under that parent, and recomputes ready labels from current state. Native dependencies are authoritative when available; the configured fallback is used only when native dependency data is unavailable. A child is ready only when open, unblocked by every blocker, and unassigned. Remove stale ready labels from blocked, assigned, closed, or otherwise non-executable children. Preserve parent order, make no parent-state or issue-resolution changes, and report promotions, removals, no-ops, unresolved blockers, and the resulting frontier. If the trigger is open, stop without mutations and report the failed `Closes #<n>` lifecycle.
