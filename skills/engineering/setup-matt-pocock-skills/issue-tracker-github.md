...
- **`/reconcile` contract**: `/reconcile <ticket-ref>` uses the resolved ticket as its trigger, follows its direct parent/spec, and enumerates every implementation child under that parent before recomputing ready labels from current state. Prefer the parent's native sub-issue relationship when available, preserving its order. When native child relationships are unavailable, enumerate issues in the repository whose direct `## Parent` reference resolves to that parent, and order them by the configured fallback, defaulting to ascending issue number. Include closed and open children so stale ready labels can be removed. Native dependencies are authoritative when available; the configured fallback is used only when native dependency data is unavailable. A child is ready only when open, unblocked by every blocker, and unassigned. Remove stale ready labels from blocked, assigned, closed, or otherwise non-executable children. Preserve the resolved child order, make no parent-state or issue-resolution changes, and report promotions, removals, no-ops, unresolved blockers, and the resulting frontier. If the trigger is open, stop without mutations and report the failed `Closes #<n>` lifecycle.
