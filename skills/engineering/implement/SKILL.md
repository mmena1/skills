...
5. Resolve the repository's default branch from `refs/remotes/origin/HEAD` (falling back to the repository's explicitly documented default branch when that symbolic ref is unavailable). If the default branch cannot be resolved, stop before changing the worktree or tracker. Then inspect the current branch with `git branch --show-current`.
...
For a ticket run, perform the configured tracker closeout exactly as `docs/agents/issue-tracker.md` describes. Local tickets may be resolved and their frontier refreshed. For GitHub issues, record the implementation commit and verification evidence but leave the issue open; ensure the PR or its merge commit carries the configured `Closes #<issue>` reference so GitHub closes it only after merge. Do not promote GitHub-dependent frontier tickets until that issue is actually closed after merge. Report the ticket or spec, approved authority, `BASELINE`, commits, verification, review result, and any newly available frontier work.
