...
- If it has no parent/spec, resolve standalone authority only from the complete tracker record defined in `docs/agents/issue-tracker.md`, and verify the approval comment's human account and repository permission. Accept effective `write` (including `maintain`) or `admin`; reject bots and fail closed when permission cannot be verified. `author_association` is supplemental context only. A readiness label, brief, or conversation history is not that record.
