...
4. **Grill (if needed).** If the request needs fleshing out, apply `grilling`, then apply `domain-modeling`, and grill it into shape a round of questions at a time, sharpening domain terms and updating `CONTEXT.md`/ADRs inline as decisions land. If either skill is not already loaded and the harness exposes skill files, load and follow its `SKILL.md` first.
