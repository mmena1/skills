...
| **`/handoff-doc`** | Write a portable markdown file and seed a session anywhere with it. |
...
**1. Can you continue in this session?** Two things make the answer yes: the next phase needs this phase as a **primary source**, or you have enough smart zone left (about 150k tokens) for the next phase to fit. Grilling to implementation is the standard yes: the implementation wants the reasoning verbatim, not a summary of it. Continue costs nothing and loses nothing, so rule it out before anything else.
...
**3. Do you need to hand off?** `/handoff-doc` is narrow. You need it only when you are:
...
That list is the whole clause. What `/handoff-doc` buys is **portability**: a file that travels. If nothing is travelling, you don't need it.
...
| Secondary (`/compact`, `/handoff-doc`) | Lossy       | Less  | Lots         |
